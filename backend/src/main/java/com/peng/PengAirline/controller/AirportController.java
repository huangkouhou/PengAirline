...
public class AirportController {
    ...
    public ResponseEntity<Response<?>> updateAirport(
            ...
            @Valid @RequestBody AirportUpdateDTO airportUpdateDTO) {
        return ResponseEntity.ok(airportService.updateAirport(id, airportUpdateDTO));
    }


    @GetMapping // 不带参数的 GET → 获取集合资源 GET 方法只是读取，不会改变系统状态，因此不需要加 @PreAuthorize
    public ResponseEntity<Response<List<AirportCreateDTO>>> getAllAirports() {
        return ResponseEntity.ok(airportService.getAllAirports());
    }

    @GetMapping("/{id}") // 带路径参数（/{id}） 的 GET → 获取单个资源
    public ResponseEntity<Response<AirportCreateDTO>> getAirportById(@PathVariable Long id) {
